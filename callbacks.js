...
let contains = (arr,food,cb) => {cb(arr.includes(food));};
...
removeDuplicates = (arr,cb) => {cb(arr.filter((food,pos,a) => a.indexOf(food) === pos));};
removeDuplicates(foods, function(uniqueFoods) {
  console.log('foods with duplicates removed: ' + uniqueFoods);
});

// Write a function called forEach that iterates over the provided array and passes the value and index into the callback.
let forEach = (arr,cb) => {(arr.forEach((food,i) => cb(food,i)))};
